...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const startAutoClick = function () {
    const farmButtons = $('.farmGod_icon');
    let delay = 300; // 1 segundo entre cada clique
  
    function clickNext(i) {
      if (i >= farmButtons.length) return;
  
      if (game_data.market != 'nl' || $(farmButtons[i]).data('origin') == curVillage) {
        $(farmButtons[i]).trigger('click');
      }
  
      setTimeout(() => clickNext(i + 1), delay);
    }
...
// Aqui iniciamos o clique automático
startAutoClick();
